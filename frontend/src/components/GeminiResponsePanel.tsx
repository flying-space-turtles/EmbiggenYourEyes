...
const GeminiResponsePanel: React.FC<GeminiResponsePanelProps> = ({ response, isLoading, onClose }) => {
  ...
  return (
    <>
      {/* Main Panel */}
      <div className={`mt-2 p-3 bg-black/70 text-white rounded-lg shadow-lg backdrop-blur-sm border border-gray-600/30 transition-all duration-300 ${
        isExpanded ? 'fixed inset-4 z-50 overflow-auto' : 'max-h-48'
      }`}>
        ...
        <div className={`${isExpanded ? 'max-h-none' : 'max-h-32'} overflow-y-auto`}>
          {isLoading ? (
        ...
        {!isLoading && response && (
          <p className="text-xs text-gray-400 mt-2 pt-2 border-t border-gray-600/30">
